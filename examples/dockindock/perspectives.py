...
class PerspectivesManager(QObject):
    ...
    def addPerspective(self, name: str, widget: DockInDockWidget) -> None:
        if self.__perspectives_folder:
            ...
            perspective.settings = self.getSettingsObject(
                self.getSettingsFileName(name, True)
            )
            ...
            widget.getManager().removePerspectives(
                widget.getManager().perspectiveNames()
            )
    ...
    def openPerspective(self, name: str, widget: DockInDockWidget) -> None:
        ...
        if self.__perspectives_folder:
            if name in self.__perspectives:
                ...
                if widget.canCreateNewGroups():
                    ...
                    for group in self.__perspectives[name].groups.keys():
                        ...
                        for curgroup in cur_groups:
                            if curgroup.getPerspectiveGroupName() == group:
                                ...
                                break
                        if not found:
                            group = DockInDockManager.getGroupNameFromPersistGroupName(
                                group
                            )
                    ...
                    for curgroup in cur_groups:
                        if (
                            ...
                            not in self.__perspectives[name].groups.keys()
                        ):
                ...
                widget.getManager().loadPerspectivesRec(
                    self.__perspectives[name].settings
                )
            ...
            for i in range(0, size):
                ...
                if perspective:
                    ...
                    for key in main_settings.allKeys():
                        self.__perspectives[perspective].groups[key] = (
                            main_settings.value(key)
                        )
        ...
        if self.__perspectives_folder:
            ...
            for i, perspective in enumerate(self.__perspectives.keys()):
                ...
                for group in self.__perspectives[perspective].groups.keys():
                    main_settings.setValue(
                        group, list(self.__perspectives[perspective].groups[group])
                    )
                main_settings.endGroup()
            main_settings.endArray()
