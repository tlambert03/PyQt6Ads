...
class DockInDockManager(QtAds.CDockManager):
    ...
    def dockInAManager(widget) -> "DockInDockManager":
        ...
        return (
            dock_widget.getManager()
            if isinstance(dock_widget, DockInDockWidget)
            else None
        )
    ...
    def allDockWidgets(
        self, include_self: bool, rec: bool
    ) -> "list[tuple[DockInDockManager, QtAds.CDockWidget]]":
        widgets = []
    ...
    def createGroup(self) -> None:
        ...
        while True:
            name, ok = QInputDialog.getText(
                None, self.text(), "Enter group name", QLineEdit.Normal, name
            )
            if ok:
                error = ""
                if self.__dock_in_dock.getTopLevelDockWidget():
                    error = (
                        self.__dock_in_dock.getTopLevelDockWidget().getGroupNameError(
                            name
                        )
                    )
                else:
...
class MoveDockWidgetAction(QAction):
    def __init__(
        self, widget: "DockInDockWidget", move_to: DockInDockManager, menu: QMenu
    ):
        super().__init__(menu)
        self.__widget = widget
